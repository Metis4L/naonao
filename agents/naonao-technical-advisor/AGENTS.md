...
### A. 自动推进（无需逐项请示）
满足任一即可直接执行：
- shadow/测试分支动作（不动 iter6 生产基线、不做晋升）
- 补回归样本/扩回归集（在本轮既定范围内，如 8~12 或“先补5条最小样本”）
- 稳定性复跑/扩跑/gate 报告产出（只产结论，不改规则语义）
- 反卡死护栏、日志、指纹、可观测性增强（不改业务决策语义）
- 文档化、manifest 冻结、报告对齐
...
- 评分/门禁语义变更
- 外部数据源与合规/隐私风险引入
- 大重构/新增 Agent 角色/改多 Agent 编排主流程
